...
export const uploadImage = async (file: File): Promise<string> => {
  ...
  const response = await axiosInstance.post(API_ENDPOINTS.STORAGE.IMAGE, formData, multipartConfig);

  return response.data?.data?.imageName;
};

export const uploadImageGetUrls = async (file: File): Promise<{
  imageName: string;
  originalUrl: string;
  mediumUrl: string;
  smallUrl: string;
...
export const deleteImage = async (fileName: string): Promise<void> => {
  await axiosInstance.delete(`${API_ENDPOINTS.STORAGE.DELETE_IMAGE}/${fileName}`);
};
