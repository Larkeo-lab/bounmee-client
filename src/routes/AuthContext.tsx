...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
    ...
    const checkAuthStatus = async () => {
      try {
        ...
        if (userDataStr) {
          ...
          if (userData?.accessToken && !checkTokenExpired(userData.accessToken)) {
            setUser(userData)
            setIsAuthenticated(true)
          } else {
            // Token expired or invalid, clear storage
            localStorage.removeItem('authOdsc')
            setUser(null)
  ...
  const login = async (userData: { username: string, password: string }) => {
    ...
    localStorage.setItem('authOdsc', JSON.stringify(response?.data))
  }
