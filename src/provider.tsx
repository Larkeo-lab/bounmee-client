...
export const Provider: React.FC<{ children: React.ReactNode }> = ({
  ...
  useEffect(() => {
    const tableIdsToClear = Object.entries(carts)
      .filter(
        ([id, items]) => (!items || items.length === 0) && dismissedCarts[id],
      )
      .map(([id]) => id);
  ...
  const mergeCarts = useCallback(
    (local: CartItem[], incoming: any[], tableStatus?: string): CartItem[] => {
      ...
      (local || []).forEach((localItem) => {
        ...
        const key =
          `${localItem.id}-${localStatus}-${(localItem.note || "").trim()}`.toUpperCase();
  ...
  const updateQuantity = useCallback(
    (id: string, status: string, delta: number, note?: string) => {
      updateCurrentCart((prev) =>
        prev.map((item) => {
          if (
            item.id === id &&
            ...
            if (
              ...
              toast.error(
                `ສິນຄ້າ "${item.name}" ມີໃນສະຕັອກພຽງ ${item.stockQty} ລາຍການ`,
              );
  ...
  const setQuantity = useCallback(
    (id: string, status: string, value: string, note?: string) => {
  ...
  const updateStatus = useCallback(
    (uniqueIds: string[], status: string, tableId?: string) => {
      ...
      updateCurrentCart((prev) => {
        ...
        if (existingIndex !== -1) {
          newCart[existingIndex] = {
            ...newCart[existingIndex],
            quantity: newCart[existingIndex].quantity + targetItem.quantity,
          };
  ...
  const dismissTable = useCallback(
    (tableId: string) => {
  ...
  const setTableCart = useCallback(
    (tableId: string, cart: CartItem[], tableStatus?: string) => {
      setCarts((prev) => {
        const mergedCart = mergeCarts(prev[tableId] || [], cart, tableStatus);

        if (JSON.stringify(prev[tableId] || []) === JSON.stringify(mergedCart))
          return prev;
  ...
  const orderingCount = Object.entries(carts).reduce(
    (acc, [tableId, items]) => {
