...
interface UseCartSyncProps {
  ...
  setCarts: React.Dispatch<React.SetStateAction<{ [tableId: string]: CartItem[] }>>;
  mergeCarts: (local: CartItem[], incoming: any[], tableStatus?: string) => CartItem[];
  isConnected: boolean;
}

export const useCartSync = ({ carts, setCarts, mergeCarts, isConnected }: UseCartSyncProps) => {
    ...
    const setupSocket = () => {
      try {
        ...
        if (currentStoreId) {
            ...
            setCarts((prev) => {
              ...
              items.forEach((newItem: any) => {
                ...
                const existingIndex = newCart.findIndex(
                  (i) => i.id === newItem.id && i.status === status && (i.note || null) === note
                );
                
                if (existingIndex > -1) {
                  newCart[existingIndex] = {
                    ...newCart[existingIndex],
                    quantity: Number(newCart[existingIndex].quantity) + Number(newItem.quantity)
                  };
                } else {
                  newCart.push({
                    ...newItem,
          ...
          socket.on("TABLE_CART_UPDATED", (data: { tableId: string, cart: any[], tableStatus?: string }) => {
            setCarts((prev) => {
              const localCart = prev[data.tableId] || [];
              const mergedCart = mergeCarts(localCart, data.cart, data.tableStatus);
          ...
          socket.on("CUSTOMER_UPDATE_QTY", ({ tableId, index, delta }) => {
            setCarts((prev) => {
              const tableCart = prev[tableId] || [];
              const newCart = [...tableCart];
              if (newCart[index] && (newCart[index].status === "PENDING" || !newCart[index].status)) {
                const newQty = Math.max(0, (newCart[index].quantity || 0) + delta);
  ...
  useEffect(() => {
    try {
      ...
      if (!authData) return;
      const authDataJson = JSON.parse(authData);
      const storeId = authDataJson?.user?.store?.id || authDataJson?.user?.storeId;
      ...
      Object.entries(carts).forEach(([tableId, items]) => {
        ...
        if (lastSyncRef.current[tableId] !== currentJson) {
          ...
          socket.emit("SYNC_TABLE_CART", { storeId, tableId, cart: items }, (res: any) => {
            if (!res?.success) {
              console.error("❌ Sync failed for table", tableId, res?.error);
              toast.error(`ສຳຮອງຂໍ້ມູນລົ້ມເຫລວ (Sync Failed): ໂຕະ ${tableId}`);
