...
export function showErrorToast(error: any, customMessage?: string, color?: ToastColor) {
    let errorCode = ''
    ...
    const config = DEFAULT_ERROR_MESSAGES[errorCode as keyof typeof DEFAULT_ERROR_MESSAGES]

    toastGlobal({
        title: "ເກີດຂໍ້ຜິດພາດ!",
        description: config?.message || fallbackMessage || (typeof error === 'string' ? error : "ກະລຸນາລອງໃໝ່ພາຍຫຼັງ"),
        color: color || 'danger'
    })
}
