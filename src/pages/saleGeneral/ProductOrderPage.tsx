...
export default function ProductOrderPage() {
  ...
  useEffect(() => {
    ...
    const onScanned = (product: Product) => {
      addToCart(product);
      toast.success(`ຍິງບາໂຄດ: ເພີ່ມ ${product.name} ສຳເລັດ`, { duration: 1000 });
    };
  ...
  } = useGetProducts(user?.user?.storeId || "", selectedCategory === "all" ? undefined : selectedCategory, true, debouncedSearch);
  ...
  const handleAddToCart = (product: Product, event: any) => {
    ...
    if (!target) {
      toast.success(`ເພີ່ມ ${product.name}`, { duration: 800, position: "top-center" });
      return;
    }
  ...
  const categories = [
    { id: "all", label: "ທັງໝົດ" },
    ...(categoryResponse?.data?.map((cat: Category) => ({ id: cat.id, label: cat.name })) || []),
  ];
    ...
    <div className="flex flex-col lg:flex-row h-[calc(100vh-100px)] gap-0 lg:gap-4 overflow-hidden sm:m-4 relative">
      <div className="flex-grow flex flex-col min-h-0 lg:h-full">
        ...
        <div className="flex flex-col gap-2 lg:gap-4 bg-white dark:bg-gray-800 p-2 lg:p-4 rounded-xl shadow-sm border-b lg:border border-divider z-10">
          <div className="flex items-center gap-3">
             ...
             <div className="flex-grow">
                <Input
                  isClearable size="sm" variant="bordered" className="w-full lg:max-w-[400px]"
             ...
             <div className="font-black text-lg text-primary lg:block hidden">Dee POS</div>
             <Badge color="danger" content={cart.length} isInvisible={cart.length === 0} shape="circle" size="md" className="lg:hidden">
                <div className="p-2 bg-primary/10 rounded-full cursor-pointer" onClick={() => setIsMinimized(false)}>
                  <ShoppingCart size={22} className="text-primary" />
                </div>
              </Badge>
          </div>

          <ScrollShadow size={40} orientation="horizontal" className="max-w-full w-0 min-w-full overflow-x-auto scrollbar-hide" hideScrollBar>
            <Tabs aria-label="Product Categories" color="primary" variant="underlined" selectedKey={selectedCategory} onSelectionChange={(key) => setSelectedCategory(key as string)}
              classNames={{
                ...
                tabContent: "group-data-[selected=true]:text-primary font-medium text-xs lg:text-sm whitespace-nowrap",
              }}
            >
              {categories.map((cat) => <Tab key={cat.id} title={cat.label} />)}
            </Tabs>
          </ScrollShadow>
        </div>
        ...
        <div className={clsx(
          ...
          cart.length > 0 && !isMinimized ? "pb-[45vh] lg:pb-1" : "pb-[80px] lg:pb-1"
        )}>
          {!isLoadingProducts && products.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full py-10">
               <EmptyState message="ບໍ່ພົບລາຍການສິນຄ້າ" description="ລອງຄົ້ນຫາດ້ວຍຄຳສັບອື່ນ ຫຼື ປ່ຽນໝວດໝູ່" />
            </div>
          ) : (
            <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-2 lg:gap-4">
              {products.map((product) => (
                <Card isPressable key={product.id} onPress={(e) => handleAddToCart(product, e as any)}
                ...
                >
                  <CardBody className="p-0 relative overflow-hidden h-[100px] sm:h-[120px] lg:h-[140px]">
                    <div className="absolute top-1.5 right-1.5 z-20">
                      <div className={clsx("px-2 py-0.5 rounded-full text-[9px] lg:text-[10px] font-bold text-white shadow-lg backdrop-blur-md", product.stockQty > 10 ? "bg-green-500/80" : product.stockQty > 0 ? "bg-orange-500/80" : "bg-red-500/80")}>
                    ...
                    <Image shadow="none" radius="none" width="100%" alt={product.name} className="w-full object-cover h-full group-hover:scale-110 transition-transform duration-500" src={getDisplayImageUrl(product.image)} />
                  </CardBody>
                  <CardFooter className="flex flex-col items-start gap-0.5 p-2 bg-white/40 dark:bg-gray-900/40 backdrop-blur-sm">
                    <b className="text-[11px] lg:text-[12px] font-bold text-default-700 w-full truncate group-hover:text-primary transition-colors">{product.name}</b>
                    <p className="text-primary font-black text-[12px] lg:text-[14px] whitespace-nowrap">{formatNumber(product.price)} <span className="text-[8px] lg:text-[9px] font-medium text-default-400">ກີບ</span></p>
                  </CardFooter>
                </Card>
              ))}
            </div>
      ...
      {cart.length > 0 && isMinimized && (
         <Button color="primary" className="fixed bottom-6 right-6 lg:hidden z-50 rounded-full h-14 w-14 shadow-2xl animate-in zoom-in duration-300 min-w-0 p-0" onClick={() => setIsMinimized(false)}>
      ...
      <OrderRight isMinimized={isMinimized} setIsMinimized={setIsMinimized} onPaymentOpen={onOpen} />

      <PaymentModal isOpen={isOpen} onOpenChange={onOpenChange} total={subtotal} items={cart} onPaymentSuccess={() => { clearCart(); refetchProducts(); }} />
      ...
      <div className="fixed inset-0 pointer-events-none z-[9999] overflow-hidden">
        <AnimatePresence>
          {flyingItems.map(item => (
            <motion.div
              key={item.id}
              initial={{ x: item.startX, y: item.startY, scale: 0.8, opacity: 1 }}
              animate={{ 
                x: window.innerWidth > 1024 ? window.innerWidth - 100 : window.innerWidth - 60, 
                y: window.innerWidth > 1024 ? window.innerHeight / 2 : window.innerHeight - 60,
                scale: 0.2,
                opacity: 0,
                rotate: 720
              }}
            ...
            >
              <div className="w-20 h-20 rounded-2xl overflow-hidden border-2 border-primary shadow-2xl bg-white p-1">
                <img src={item.src} className="w-full h-full object-cover rounded-xl" alt="flying" />
              </div>
            </motion.div>
          ))}
        </AnimatePresence>
